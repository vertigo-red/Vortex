...
export class BSAArchive {
  ...
  async extractFile(file: BSAFileEntry, outputPath: string): Promise<void> {
    ...
    try {
      ...
      const outFile = await prepareArchiveOutputPath(outputPath, file.name);
  ...
  async extractAll(outputPath: string): Promise<void> {
    ...
    try {
      for (const file of this.fileList) {
        ...
        const outFile = await prepareArchiveOutputPath(outputPath, file.fullPath);
        await fs.promises.mkdir(path.dirname(outFile), { recursive: true });
